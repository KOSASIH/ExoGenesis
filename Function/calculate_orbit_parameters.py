...
def calculate_satellite_position(
    ...
    satellite_position = calculate_position_from_orbital_elements(
        period,
        semi_major_axis,
        semi_minor_axis,
        eccentricity,
...
def calculate_position_from_orbital_elements(
    period,
    semi_major_axis,
    semi_minor_axis,
    eccentricity,
    ...
    theta = math.atan2(
        ...
        r * (1 - eccentricity**2) / (1 + eccentricity * math.cos(eccentric_anomaly))
        - 1,
    )
    phi = math.atan2(
        math.sin(raan) * math.sin(theta)
        - math.sin(inclination) * math.cos(raan) * math.cos(theta),
        math.cos(raan) * math.sin(theta)
        + math.sin(inclination) * math.sin(raan) * math.cos(theta),
    )
    lambda_ = math.atan2(
        math.sin(theta) * math.cos(raan)
        + math.sin(inclination) * math.sin(raan) * math.sin(theta),
        math.cos(theta),
    )
    x = r * (
        math.cos(lambda_) * math.cos(phi)
        - math.sin(lambda_) * math.sin(phi) * math.cos(inclination)
    )
    y = r * (
        math.cos(lambda_) * math.sin(phi)
        + math.sin(lambda_) * math.cos(phi) * math.cos(inclination)
    )
...
def automate_satellite_deployment(satellite_data, launch_site, launch_vehicle):
    ...
    (
        ...
        raan,
        true_anomaly,
    ) = calculate_orbit_parameters(altitude, semi_major_axis)
